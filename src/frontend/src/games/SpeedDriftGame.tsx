...
function drawPlayerCar(
  ...
  ctx.beginPath();
  ctx.roundRect(-hw + 3, -hh, PLAYER_W - 6, PLAYER_H, 6);
  ctx.fill();
  ctx.shadowBlur = 0;

  ctx.fillStyle = "rgba(0,220,255,0.5)";
  ctx.beginPath();
  ctx.roundRect(-hw + 6, -hh + 8, PLAYER_W - 12, 16, 3);
  ctx.fill();
...
export default function SpeedDriftGame({ onGameOver }: Props) {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    for (let screenY = H; screenY >= 0; screenY--) {
      ...
      ctx.fillStyle = screenY % 40 < 20 ? "#0a1a08" : "#0c1f0a";
      ctx.fillRect(0, screenY, W, 1);

      ctx.fillStyle = screenY % 60 < 30 ? "#222" : "#1e1e1e";
    ...
    for (const obs of s.obstacles) {
      ...
      if (!seg) continue;
      drawCone(ctx, seg.x + obs.offsetX, screenY);
    }
    ...
    ctx.textAlign = "right";
    ctx.fillStyle = s.timeLeft < 10 ? "#FF4C1A" : "#00C8FF";
    ctx.fillText(`TIME: ${Math.ceil(s.timeLeft)}`, W - 10, 23);
    ctx.textAlign = "left";

    if (!s.onRoad) {
      ctx.fillStyle = "rgba(255,76,26,0.15)";
      ctx.fillRect(0, 0, W, H);
